...
test_duping1 <- expand_grid(NS_X = picnic_id[which( picnic_id$transect_orientation == "NS"), "x"]$x, 
              EW_X = picnic_id[which( picnic_id$transect_orientation == "EW"), "x"]$x) %>% 
  add_column(
  expand_grid(NS_Y = picnic_id[which( picnic_id$transect_orientation == "NS"), "y"]$y, 
              EW_Y = picnic_id[which( picnic_id$transect_orientation == "EW"), "y"]$y)) %>%
  filter(abs(NS_X - EW_X) <= 0.5 & abs(NS_Y - EW_Y) <= 0.5)

picnic_id[which(sapply(c(picnic_id$x), FUN = function(x)x %in% c(test_duping1$NS_X )) & 
                sapply(c(picnic_id$y), FUN = function(x)x %in% c(test_duping1$NS_Y )) ), ]

  
picnic_id[which((sapply(c(picnic_id$x), FUN = function(x)x %in% c(test_duping1$EW_X )) & 
                 sapply(c(picnic_id$y), FUN = function(x)x %in% c(test_duping1$EW_Y ))) |
                (sapply(c(picnic_id$x), FUN = function(x)x %in% c(test_duping1$NS_X )) & 
                 sapply(c(picnic_id$y), FUN = function(x)x %in% c(test_duping1$NS_Y )) )
                ),]
