...
if(
  ( abs(fram_id[i,'x'] - fram_id[n,'x']) <= 0.05 ) & 
    abs(fram_id[i,'y'] - fram_id[n,'y']) <= 0.05 ){
...
duping <- function(site, b){

test_duping1 <- expand_grid(NS_X = site[which( site$transect_orientation == "NS"), "x"]$x, 
...
site[which((sapply(c(site$x), FUN = function(x)x %in% c(test_duping1$EW_X )) & 
            sapply(c(site$y), FUN = function(x)x %in% c(test_duping1$EW_Y )))|
           (sapply(c(site$x), FUN = function(x)x %in% c(test_duping1$NS_X )) & 
            sapply(c(site$y), FUN = function(x)x %in% c(test_duping1$NS_Y )) )
                ),]
}
...
fram_duped <- duping(fram_id, .5)
mutate(fram_duped, plant_id = case_when( abs))
...
NS_dupe <- test %>% 
  select(NS_X, NS_Y) %>% 
  mutate( plant_id = case_when((sapply(c(site$x), FUN = function(x)x %in% c(NS_X )) & 
                                sapply(c(site$y), FUN = function(x)x %in% c(NS_Y ))) ~ 
                                site[which((sapply(c(site$x), FUN = function(x)x %in% c(NS_X )) & 
                                            sapply(c(site$y), FUN = function(x)x %in% c(NS_Y )) )), "plant_id"], TRUE ~ 0))
NS_dupe
}
...
site[which((sapply(c(site$x), FUN = function(x)x %in% c(test$EW_X )) & 
            sapply(c(site$y), FUN = function(x)x %in% c(test$EW_Y )))
           |
           (sapply(c(site$x), FUN = function(x)x %in% c(test$NS_X )) & 
            sapply(c(site$y), FUN = function(x)x %in% c(test$NS_Y )) )
                ),]
}
...
sapply(c(fram_id[which( fram_id$transect_orientation == "EW"), "x"]$x), 
       FUN = function(U){
         sapply(c(fram_id[which( fram_id$transect_orientation == "NS"), "x"])$x,
                FUN = function(V){
                  sum(which(abs( V - U ) <= 0.5))} 
         )
       }
)
