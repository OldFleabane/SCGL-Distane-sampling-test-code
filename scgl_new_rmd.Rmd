...
test <- expand_grid(NS_X = fram_crazy[which( fram_crazy$transect_orientation == "NS"), "x"]$x, 
                    EW_X = fram_crazy[which( fram_crazy$transect_orientation == "EW"), "x"]$x) %>% 
  add_column(
        expand_grid(NS_Y = fram_crazy[which( fram_crazy$transect_orientation == "NS"), "y"]$y, 
                    EW_Y = fram_crazy[which( fram_crazy$transect_orientation == "EW"), "y"]$y)) %>% 
  filter(abs(NS_X - EW_X) <= fuzz & abs(NS_Y - EW_Y) <= fuzz)

test_join <- full_join(fram_crazy, test, by = join_by(x == EW_X, y == EW_Y))
...
fram_crazy <- test_dupe


temp_error <-  which(error == fuzz)  
   error[which(error == fuzz)] <- ( sum(duplicated(fram_crazy$plant_id)) / nrow(fram_crossed))
