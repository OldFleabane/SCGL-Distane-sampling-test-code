...
pic_coord1 %>% 
  ...
  geom_point(data =  pic_ns[which(pic_ns$plant_id %in% pic_ew$plant_id),], aes(colour = "black")) +
  geom_point(data = pic_ew[which(pic_ew$plant_id %in% pic_ns$plant_id),], aes(colour = "black")) + 
...
pic_coord1 %>% 
 ...
 pic_coord %>% 
   filter(abs(x)<100, abs(y)<100) %>% 
   ggplot( mapping = aes( x = x , y = y, colour = transect_orientation))+
   geom_point()
```
