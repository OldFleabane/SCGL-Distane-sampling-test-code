...
need to loop through NS observations and look for matching EW observations based on a fuzziness of the (x,y). 
Ideally Going with fuzziness of 0.1m because any cactus within 10cm of another was considered part of the same group during the survey.
Will play with fuzziness and see the crosses
...
if(
  ( abs(fram_ns$x[i] - fram_ew$x[n]) <= 0.25 ) &  #Check for overlap, at 10cm only 1 observation.
  abs(fram_ns$y[i] - fram_ew$y[n]) <= 0.25 ){
...
three_test %>% 
  ...
  geom_point( data = three_both, aes())
