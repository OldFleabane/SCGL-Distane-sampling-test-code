...
Use a Purr functions to map a scoring function on the list of unique observation pairs?
...
test.fun <- function(site){
  ...
  list <-  lapply(list[c(1001, 2565, 3000, 10, 156, 785)], function(z){
    
    #indexing data with the paired global_id's
    a <- site[which(site$global_id == z[1]),]
    b <- site[which(site$global_id == z[2]),]
   
    #Calculating distance
    z[3] <- sqrt(sum((c(a$x,a$y) - c(b$x,b$y))^2))
    
    #determing if they are on the same transect
    z[4] <- if(a$transect_orientation == b$transect_orientation) "same transect" else "diff transect"
  ...
  df <- bind_rows(lapply(list, function(z.1){
           data.frame("id_1" = z.1[1],
                      "id_2" = z.1[2],
                      "distance" = z.1[3],
                      "transect" = z.1[4])
...
#this makes the list of all combos of 2 global_id's into a Dataframe
#as.data.frame(t(combn(fram_1$global_id, m = 2, simplify = TRUE)))

#fram_1[which(fram_1$global_id == combn(fram_1$global_id, m = 2, simplify = FALSE)[[2065]][1]),]
