...
# - Number in group
# - was reproductive (flowering)
# - distance between observations

#function to apply comparison to each object_id pair by referencing the fram_1 table
#Needs to use the expand.grid pairs and parse the fram_1 table and then compare
#the covariates to evaluate a probability score
...
test.fun <- function(site){
  ...
  list <- lapply(list, function(list.1){
    ...
    a <- site[site$global_id == list.1[1],]
    b <- site[site$global_id == list.1[2],]
    ...
    list.1[5] <- paste(sum(select(a, contains("number_")), na.rm = TRUE),
                  ", " ,
                  sum(select(b, contains("number_")), na.rm = TRUE),
                  sep = "")
...
fram_duped <- crossed(filter(fram_id), 0.5)  # a = plot data w/ID's, b = fuzziness
...
### 2026 Function testing 

```{r}
fram_new <- test.fun(fram_1)
...
```{r}
# #Trying out data.tables 
# install.packages("data.table")
# library("data.table")
# 
# input <- data.table::setDT(fram_1)
# 
#   
#  input1 <- utils::combn(input[,global_id], m = 2, simplify = TRUE) %>% 
#    t() %>% 
#    as.data.table()
#  
#  mapply(function(a,b){
#    input1[, `:=`( distance =
#                           dist(x= rbind(input[global_id %in% b, c(x,y)],
#                                         input[global_id %in% a, c(x,y)]),
#                                method = "euclidean")}
#                transect = mapply(function(a,b){
#                                  case_when(input[global_id %in% a, transect_orientation] == input[global_id %in% b, transect_orientation] ~ "same",
#                                     a$transect_orientation != b$transect_orientation ~"different")
#                )]
...
```{r}
#testing new function at picnic

test_df1 <- picnic_1 %>% 
  test.fun()
  
filter(test_df1, distance == min(distance), .by = c(id_2))

(function(x){
  cutoffs <- seq(0, 1, by = 0.01)
   duplicates <- sapply(cutoffs, function(y){
    nrow(filter(x, distance < y))
    })
   data.frame(cutoffs, duplicates)
})(test_df1) %>% 
  
  ggplot( mapping = aes(x = cutoffs, y = duplicates)) +
  geom_point()
