...
fram_id <- crossed(filter(fram_id, zoned == 1), 0.5)  # a = plot data w/ID's, b = fuzziness
...
ob_error_fram <- (nrow(filter(fram_id, duplicated(plant_id))) / nrow(filter(fram_id, zoned == 1)))
```
...
picnic_id <- crossed(filter(picnic_id, zoned == 1), 0.5)
...
ob_error_picnic <- (nrow(filter(picnic_id, duplicated(plant_id))) / nrow(filter(picnic_id, zoned == 1)))
...
picnic_crazy <- mutate(picnic_crazy, zoned = case_when( c(rowSums(mapply( FUN = function(a, b){between(a, b-3, b+3)}, picnic_crazy[, "y"], c(20, 30, 40, 50))) +
                                                    rowSums(mapply( FUN = function(a, b){between(a, b-3, b+3)}, picnic_crazy[, "x"], c(-20, -30, -40)))) == 2 ~ 1,
       TRUE ~ 0)) 

 temp_site <-  picnic_crazy %>% 
    rowwise() %>% 
    mutate(coord = list(c(x,y)))
  
 grid <-  expand.grid(EW = temp_site[which( temp_site$transect_orientation == "EW"), "coord"]$coord,
                      NS = temp_site[which( temp_site$transect_orientation == "NS"), "coord"]$coord) %>% 
    rowwise() %>% 
    mutate( dist = sqrt(sum((EW - NS)^2))) %>% 
    filter(dist <= fuzz) 
