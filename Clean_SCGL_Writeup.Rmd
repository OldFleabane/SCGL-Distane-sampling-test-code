...
picnic_fuzz <- tibble( fuzziness = c(seq(0, 1, by = 0.1)))
 picnic_fuzz <-  picnic_fuzz %>% 
    mutate( error = numeric(nrow(picnic_fuzz)), 
            n_duplicated = numeric(nrow(picnic_fuzz)),
            n_actual = numeric(nrow(picnic_fuzz)), 
            n_corrected = numeric(nrow(picnic_fuzz))
    )
    



for (fuzz in picnic_fuzz$fuzziness){
...
if(
  ( abs(picnic_crazy[i,'x'] - picnic_crazy[n,'x']) <= fuzz ) & 
    ...
    picnic_fuzz[which(as.character(picnic_fuzz$fuzziness) == as.character(fuzz)), "error"] <- (nrow(filter(picnic_crazy, duplicated(plant_id))) / nrow(picnic_crossed))

    picnic_fuzz[which(as.character(picnic_fuzz$fuzziness) == as.character(fuzz)), "n_duplicated"] <- sum(duplicated(picnic_crazy$plant_id))
    
    picnic_fuzz[which(as.character(picnic_fuzz$fuzziness) == as.character(fuzz)), "n_actual"] <- n_distinct(picnic_crazy$plant_id)
    
    picnic_fuzz[which(as.character(picnic_fuzz$fuzziness) == as.character(fuzz)), "n_corrected"] <- mean( c(n_distinct(which(picnic_crazy$transect_orientation == "NS"))/with(picnic_fuzz, error[ fuzziness == fuzz] ), n_distinct(which(picnic_crazy$transect_orientation == "EW"))/with(picnic_fuzz, error[ fuzziness == fuzz] )))
