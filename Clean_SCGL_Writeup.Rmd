...
test <- expand_grid(NS_X = picnic_crazy[which( picnic_crazy$transect_orientation == "NS"), "x"]$x, 
                    EW_X = picnic_crazy[which( picnic_crazy$transect_orientation == "EW"), "x"]$x) %>% 
  add_column(
        expand_grid(NS_Y = picnic_crazy[which( picnic_crazy$transect_orientation == "NS"), "y"]$y, 
                    EW_Y = picnic_crazy[which( picnic_crazy$transect_orientation == "EW"), "y"]$y)) %>% 
  filter(abs(NS_X - EW_X) <= fuzz & abs(NS_Y - EW_Y) <= fuzz)

test_join <- full_join(picnic_crazy, test, by = join_by(x == EW_X, y == EW_Y))
...
picnic_crazy <- test_dupe


temp_error <-  which(error == as.character(fuzz))  
   error[which(error == as.character(fuzz))] <- (nrow(filter(picnic_crazy, duplicated(plant_id))) / nrow(pic_crossed))
